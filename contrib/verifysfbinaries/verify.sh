...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'astrocoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[astrocoin-]<version>-[rc[0-9]] (example: astrocoin-0.7.1-rc1)"
   echo "wget output:"
